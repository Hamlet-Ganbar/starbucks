import Routing from './Routing/Routing';
import { MainContext } from './Components/Context/Context';
import { useState } from 'react';
import { GlobalProducts } from './Components/Context/ProductsContext';

function App() {
    const data = [
        {
            category: 'Featured',
            see: 'See all',
            cards: [
                "https://globalassets.starbucks.com/assets/aee2fd4b97ef4cfea2adbc321b2739ac.jpg",
                "https://globalassets.starbucks.com/assets/d546023396854e3dbe1d340a21825543.jpg",
                "https://globalassets.starbucks.com/assets/57bd5a19dbe64a30b4f22d7ad6ece127.jpg",
                "https://globalassets.starbucks.com/assets/a77dc2f2976e492686e6e9df7a7d5cc7.jpg",
                "https://globalassets.starbucks.com/assets/14206ea6e74a43c6a0a5d4abc81e95bb.jpg",
                "https://globalassets.starbucks.com/assets/d0df28dbc08f478eba00b26ca09bffe1.jpg",
                "https://globalassets.starbucks.com/assets/e59a52bdd19448be9096d788b85a5d2f.jpg",
                "https://globalassets.starbucks.com/assets/8c34f576af314654956200e08429e9bd.jpg",
                "https://globalassets.starbucks.com/assets/1bf45b972317492c9a3762a67885a354.jpg",
                "https://globalassets.starbucks.com/assets/7ad856a8cbdd4fbeb0119695a8d5b843.jpg",
                "https://globalassets.starbucks.com/assets/27ade48b639f44a59ed702914a194454.jpg",
                "https://globalassets.starbucks.com/assets/025674328bc54e35a4ba0988875e6e5b.jpg",
                "https://globalassets.starbucks.com/assets/b3c8d0d8a3bc48b18d570b382af46c20.jpg",
                "https://globalassets.starbucks.com/assets/b1061db1fc834e429a9a4bfa5de5e884.jpg",
                "https://globalassets.starbucks.com/assets/1c47ac4f6e914144b8d6a2e3489f316e.jpg",
                "https://globalassets.starbucks.com/assets/6e34062e14484b66a5604c57340103ba.jpg",
                "https://globalassets.starbucks.com/assets/97be32ff56b54da5aa361cd9a6d24977.jpg",
                "https://globalassets.starbucks.com/assets/92a2acfde0d74350bae518afe9928fea.jpg"
            ]
        },
        {
            category: "Easter",
            see: '',
            cards: [
                "https://globalassets.starbucks.com/assets/aee2fd4b97ef4cfea2adbc321b2739ac.jpg",
                "https://globalassets.starbucks.com/assets/63fe1c1d9cd74192b5b28d1f0aec6602.jpg",
                "https://globalassets.starbucks.com/assets/e3775f54027147f8803f85b3bb6d4268.jpg",
                "https://globalassets.starbucks.com/assets/a77dc2f2976e492686e6e9df7a7d5cc7.jpg"
            ]
        },
        {
            category: "Siblings Day",
            see: '',
            cards: [
                "https://globalassets.starbucks.com/assets/9c48c44fafd540e9b79e72508db7698e.jpg",
                "https://globalassets.starbucks.com/assets/57bd5a19dbe64a30b4f22d7ad6ece127.jpg",
                "https://globalassets.starbucks.com/assets/5f33e3da4343452fa3922f2f41601df5.jpg"
            ]
        },
        {
            category: "Pet Lovers",
            see: '',
            cards: [
                "https://globalassets.starbucks.com/assets/538d70aafcd640e8a336b1e81db0f5ad.jpg",
                "https://globalassets.starbucks.com/assets/e6160161a69a472199628d3ea6096efe.jpg",
                "https://globalassets.starbucks.com/assets/0805c5d04491458a99629554c82f7794.jpg",
                "https://globalassets.starbucks.com/assets/d0df28dbc08f478eba00b26ca09bffe1.jpg"
            ]
        },
        {
            category: "Birthday",
            see: 'See all',
            cards: [
                "https://globalassets.starbucks.com/assets/086f127933b1492c9b6311e77d32f6ba.jpg",
                "https://globalassets.starbucks.com/assets/45f1c4ed8dac426ea31d09bc4296771c.jpg",
                "https://globalassets.starbucks.com/assets/196581be9eda4e4cb0c3b33ac3241ea7.jpg",
                "https://globalassets.starbucks.com/assets/025674328bc54e35a4ba0988875e6e5b.jpg",
                "https://globalassets.starbucks.com/assets/b3c8d0d8a3bc48b18d570b382af46c20.jpg"
            ]
        },
        {
            category: "Thank you",
            see: 'See all',
            cards: [
                "https://globalassets.starbucks.com/assets/a8fa33a190374fe69bf1de429a03053a.jpg",
                "https://globalassets.starbucks.com/assets/f8167a08eaab4d2197729d2967caedad.jpg",
                "https://globalassets.starbucks.com/assets/f8167a08eaab4d2197729d2967caedad.jpg",
                "https://globalassets.starbucks.com/assets/1bf45b972317492c9a3762a67885a354.jpg",
                "https://globalassets.starbucks.com/assets/6e34062e14484b66a5604c57340103ba.jpg"
            ]
        },
        {
            category: "Celebration",
            see: '',
            cards: [
                "https://globalassets.starbucks.com/assets/02cc779eadaf4a1891efdc9908eefb89.jpg",
                "https://globalassets.starbucks.com/assets/fc7d4cb9e0594398b00bfe133a02fad1.jpg",
                "https://globalassets.starbucks.com/assets/e59a52bdd19448be9096d788b85a5d2f.jpg",
                "https://globalassets.starbucks.com/assets/7ad856a8cbdd4fbeb0119695a8d5b843.jpg"
            ]
        },
        {
            category: "Spring",
            see: '',
            cards: [
                "https://globalassets.starbucks.com/assets/d546023396854e3dbe1d340a21825543.jpg",
                "https://globalassets.starbucks.com/assets/b1bc1467fdd644aa87a83aa96bf8e17d.jpg",
                "https://globalassets.starbucks.com/assets/be090dd487b54f4490b153e00f35136f.jpg",
                "https://globalassets.starbucks.com/assets/5e0c21687dc64afba35ab74987b5bff5.jpg"
            ]
        },
        {
            category: "Earth Month",
            see: '',
            cards: [
                "https://globalassets.starbucks.com/assets/8b136f9da31e4e4ebafd0516b448fb98.jpg",
                "https://globalassets.starbucks.com/assets/9a1c7a65cc4e4946a96dcb7fa8afa51a.jpg",
                "https://globalassets.starbucks.com/assets/14206ea6e74a43c6a0a5d4abc81e95bb.jpg"
            ]
        },
        {
            category: "Apreciation",
            see: '',
            cards: [
                "https://globalassets.starbucks.com/assets/3d9173ceb399494483d37c5c7d88958d.jpg",
                "https://globalassets.starbucks.com/assets/9e106226562b40a2b2a149db572cca58.jpg",
                "https://globalassets.starbucks.com/assets/b1061db1fc834e429a9a4bfa5de5e884.jpg",
                "https://globalassets.starbucks.com/assets/97be32ff56b54da5aa361cd9a6d24977.jpg"
            ]
        },
        {
            category: "Workplace",
            see: 'See all',
            cards:
                [
                    "https://globalassets.starbucks.com/assets/030cdc65712a4cb3b9a58483de6799b3.jpg",
                    "https://globalassets.starbucks.com/assets/c4176b3e9fe34b5087040869f703ab48.jpg",
                    "https://globalassets.starbucks.com/assets/28ae9dd37eee4f828627bd584b49f9f0.jpg",
                    "https://globalassets.starbucks.com/assets/974d46ec17774cb1bafa313d1db92bbf.jpg",
                    "https://globalassets.starbucks.com/assets/1c47ac4f6e914144b8d6a2e3489f316e.jpg"
                ]
        },
        {
            category: "ADMINISTRATIVE PROFESSIONALS' DAY",
            see: 'See all',
            cards:
                [
                    "https://globalassets.starbucks.com/assets/711e376ae60640d5a80e9f088d6d0b16.jpg",
                    "https://globalassets.starbucks.com/assets/c4176b3e9fe34b5087040869f703ab48.jpg",
                    "https://globalassets.starbucks.com/assets/afb2d5881a214b1a9ab607809586b11a.jpg",
                    "https://globalassets.starbucks.com/assets/974d46ec17774cb1bafa313d1db92bbf.jpg",
                    "https://globalassets.starbucks.com/assets/1c47ac4f6e914144b8d6a2e3489f316e.jpg"
                ]
        },
        {
            category: "Encouragement",
            see: '',
            cards:
                [
                    "https://globalassets.starbucks.com/assets/0f7b291e505e45d29fde0c4d7b732140.jpg",
                    "https://globalassets.starbucks.com/assets/c58f11e8197f4b608a1d7a751779fca3.jpg",
                    "https://globalassets.starbucks.com/assets/ff23ba2daa464e579e528cbd04749e24.jpg",
                    "https://globalassets.starbucks.com/assets/27ade48b639f44a59ed702914a194454.jpg"
                ]
        },
        {
            category: "Affection",
            see: 'See all',
            cards:
                [
                    "https://globalassets.starbucks.com/assets/0b3987211aa342b9aa1e6029570bcac3.jpg",
                    "https://globalassets.starbucks.com/assets/ef9fd7cc60c8426199db6c60e27d5d06.jpg",
                    "https://globalassets.starbucks.com/assets/76aada8fd9fd4a9d9adae18d10829062.jpg",
                    "https://globalassets.starbucks.com/assets/4d7c2ed113214af79dc5024f2073259a.jpg",
                    "https://globalassets.starbucks.com/assets/496a8765f41145299c1952a8b707ea8b.jpg"
                ]
        },
        {
            category: "Anytime",
            see: 'See all',
            cards:
                [
                    "https://globalassets.starbucks.com/assets/a224b9727cb44ea4822b459d3faa0ced.jpg",
                    "https://globalassets.starbucks.com/assets/6f956ca135ac4505818cd06be4f476e9.jpg",
                    "https://globalassets.starbucks.com/assets/d2346fc58ecb4ce39534452c5c527c11.jpg",
                    "https://globalassets.starbucks.com/assets/bb5003220b16472c8efd7c1968edb836.jpg",
                    "https://globalassets.starbucks.com/assets/92a2acfde0d74350bae518afe9928fea.jpg"
                ]
        }
    ]

    const footerData = [
        {
          title: "About Us",
          li: ["Our Company", "Our Coffe", "Stories and News", "Starbucks Archive", "Investor RElations", "Costumer Service"]
        },
        {
          title: "Careers",
          li: ["Culture and Values", "Inclusion,Diversity,and Equity", "College Achievement Plan", "Aumni Community", "U.S.Careers", "International Careers"]
        },
        {
          title: "Social Impact",
          li: ["People", "Planet", "Environmental and Social Impact Reporting"]
        },
        {
          title: "For Business Partners",
          li: ["Landlord Support Center", "Suppliers", "Corporate Gift Card Sales", "Office and Foodservice Coffee"]
        },
        {
          title: "Order And Pickup",
          li: ["Order on the App", "Order on the Web", "Delivery", "Order and Pickup Options", "Explore and Find Coffe for Home"]
        }
      ]

    const [places, setPlaces] = useState(['Sturbucks GenclikMall', 'Sturbucks Metropark', 'Sturbucks DenizMall', 'Sturbucks PortBaku',])
    const [hambur, setHambur] = useState(false)
    const [menuOpen, setMenuOpen] = useState(false)
    const [selectStore, setSelectStore] = useState(localStorage.getItem("store"))
    const [val, setVal] = useState('')


    let checkStorage = () => {
        if (localStorage.getItem('store') === null) {
            localStorage.setItem('store', '')
        }
        else {
            localStorage.getItem('store')
        }
    }

    checkStorage()

    const storeHandle = () => {
        let a = localStorage.getItem('store')
        a = places.find(item => item === val)
        localStorage.setItem('store', a)
        setSelectStore(a)
        localStorage.setItem('store', a)
    }

    const closeBar = () => {
        setMenuOpen(false)
        setHambur(false)
    }


    const datas = {
        data, footerData, hambur, setHambur, menuOpen, setMenuOpen, closeBar,
        places, setPlaces, storeHandle, selectStore, setSelectStore,
        val, setVal
    }

    return (
        <MainContext.Provider value={datas}>
            <GlobalProducts>
                <Routing />
            </GlobalProducts>
        </MainContext.Provider>
    );
}

export default App;
